...
const transormCalendars = R.compose(
  ...
  R.toPairs,
  R.mapObjIndexed((val, key, obj) => R.compose(
                  R.mapObjIndexed((v,k,o) => ({ //tidy up - just get the list of packages and url of event
                    //url is only one, group by it and drop the values - keeping the url itself only
                    url: R.compose(R.keys, R.groupBy(i => i.url))(v)[0],
...
class PackageCalendars extends Component {
  ...
  render() {
    ...
    const calendar_cards = calendars.map(c => (
      <div className="card p-2" style={{backgroundColor: "white"}} key={`package_calendar_${c[0]}`}>
        <table>
          <tbody>
            <tr>
